> **Cumplimiento:** ISO/IEC 12207, 25010, 15504, 27001, 9001.  
> **Documento versionado y auditado.**  
> **Revisión:** 2026-05-14, **Responsable:** Equipo X64DOS

# Sprint XX - [Nombre del sprint]

**Fechas:** [YYYY-MM-DD] a [YYYY-MM-DD]
**Scrum Master:** [Nombre]
**Equipo:** [Nombres]

## Objetivos
- [Listar objetivos del sprint]

## Cobertura de requisitos y reglas de negocio
- Requisitos cubiertos: [SRN-X], [SRN-Y]
- Reglas de negocio cubiertas: [RB-A], [RB-B]

## Backlog/Tareas

> **Nota de versión:** En cada entrega, el segundo campo del versionado corresponde al sprint, el tercero al issue (xx.yy.zz).

| #  | Tarea                       | Responsable | Estado        | Trazabilidad          |
|----|-----------------------------|-------------|-------------|-----------------------|
| 1  |                             |             | PENDING     | [SRN-X], [RB-A]       |
| 2  |                             |             | PENDING     | [SRN-Y], [RB-B]       |

> **Política de estados:** Solo se permiten 'PENDING', 'IN-PROGRESS' y 'DONE'.
> **Política de versiones:** El campo yy es el sprint y zz el número de issue asociado.

## Issues abiertos relacionados
- [issue-XXX](issues/issue-XXX.md): [Resumen]

## Criterios de cierre
- [Definir criterios claros y métricas de aceptación]

## Checklist de cumplimiento
- [ ] Documentación al día
- [ ] Pruebas automatizadas/QA ejecutadas
- [ ] Issues críticos resueltos
- [ ] Trazabilidad de requisitos y reglas verificada

---
*Incluye cobertura de SRN y reglas de negocio al inicio de cada sprint para trazabilidad y cumplimiento industrial.*
